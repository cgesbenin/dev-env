...
array=(geoserver-$GS_VERSION-css-plugin.zip geoserver-$GS_VERSION-printing-plugin.zip \
geoserver-$GS_VERSION-libjpeg-turbo-plugin.zip geoserver-$GS_VERSION-gdal-plugin.zip)
for i in "${array[@]}"
do
...
http://demo.geo-solutions.it/share/github/imageio-ext/releases/1.2.X/1.2.2/native/gdal/gdal-data.zip
popd
wget --progress=bar:force:noscroll -c --no-check-certificate \
http://demo.geo-solutions.it/share/github/imageio-ext/releases/1.2.X/1.2.2/native/gdal/linux/gdal192-Ubuntu12-gcc4.6.3-x86_64.tar.gz


popd
if [[ ! -f /tmp/resources/libjpeg-turbo-official_2.0.2_amd64.deb ]]; then \
    wget --progress=bar:force:noscroll -c --no-check-certificate \
    https://sourceforge.net/projects/libjpeg-turbo/files/2.0.2/libjpeg-turbo-official_2.0.2_amd64.deb \
    -P /tmp/resources;\
    fi; \
    cd /tmp/resources/ && \
...
if  [[ "$COMMUNITY_MODULES" == true ]]; then
    ...
    # geoserver-${GS_VERSION:0:4}-SNAPSHOT-backup-restore-plugin.zip geoserver-${GS_VERSION:0:4}-SNAPSHOT-authkey-plugin.zip \
    # geoserver-${GS_VERSION:0:4}-SNAPSHOT-s3-geotiff-plugin.zip geoserver-${GS_VERSION:0:4}-SNAPSHOT-gwc-s3-plugin.zip)
    array=(geoserver-${GS_VERSION:0:4}-SNAPSHOT-backup-restore-plugin.zip \
